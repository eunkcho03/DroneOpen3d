...
def depth_to_camera_points(depth, fov_degrees, far, keep_invalid_depth=False):
    ...
    y = -(v[valid_mask] - cy) * z / fy  # image y down -> camera y up
...
class DepthFrameProcessor:
    ...
    def run(self):
        ...
        # self.points_world_unfiltered = camera_to_world(
...
class ExtrusionFusionReconstruction:
    ...
    def __init__(
        ...
        #   0.0 = free / empty
        #   0.5 = unknown
        #   1.0 = occupied / directly observed
        self.free_prob = 0.0
        self.unknown_prob = 0.5
        self.occupied_prob = 1.0
        self.occupied_threshold = 0.75

        # voxel_prob stores the state of every voxel inside the initialized bbox.
        self.voxel_prob = {}

        # occupied_indices stores only voxels that are currently considered occupied.
        # Unknown voxels are kept in voxel_prob, but not plotted as occupied.
        self.occupied_indices = set()

        # observed_indices stores all directly measured surface voxels.
        self.observed_indices = set()

        # free_indices stores voxels that have been seen as empty space.
        self.free_indices = set()

        # unknown_indices stores voxels that are inside the bbox but not yet classified.
        self.unknown_indices = set()

        # Kept for compatibility with your plotting/main code.
    ...
    def add_view(
        ...
        if len(points_world) == 0:
            self._log("No object detected in this frame. Bbox is not initialized/updated.")
            return self

        if not self.initialized:
            self._initialize_bbox_from_detection(points_world)

        # Carve using the current depth image.
    ...
    def carve_with_depth_image(
        ...
        if not self.initialized:
            self._log("Cannot carve: bbox has not been initialized yet.")
            return self

        if not self.voxel_prob:
            self._log("Cannot carve: voxel probability grid is empty.")
            return self

        if carve_margin is None:
            # Half a voxel is usually a stable default.
            carve_margin = 0.5 * self.voxel_size

        depth = np.asarray(depth, dtype=float)

        if depth.ndim != 2:
            self._log("Cannot carve: depth image must be 2D.")
            return self
        ...
        # Carve all non-free voxels, including unknown voxels.
        # This is the key difference from the old solid-bbox method.
        indices = np.array([
            idx
            for idx, prob in self.voxel_prob.items()
            if prob > self.free_prob
        ], dtype=int)

        if len(indices) == 0:
            self._log("No non-free voxels left to carve.")
            return self
        ...
        # Project voxel centers into the image.
        ...
        if not np.any(projects_inside_image):
            self._log("No occupied voxels project into this view.")
            return self
        ...
        # Main carving rule:
        # If voxel depth is smaller than the measured depth, the voxel lies
        # in empty space before the observed surface, so remove it.
        carve_free_space = valid_measured_depth & (
            candidate_z < measured_depth - carve_margin
        )

        # Optional aggressive rule:
        # If the pixel has invalid/far depth, remove projected voxels before far.
        # Default is False because invalid pixels can also come from masking/noise.
        if carve_with_invalid_depth:
            invalid_or_far = ~valid_measured_depth
            carve_free_space |= invalid_or_far & (candidate_z < far - carve_margin)

        remove_indices = {
            tuple(idx)
            for idx in candidate_indices[carve_free_space]
        }

        before_free = len(self.free_indices)

        # Voxels in front of the measured surface are free.
        # Do not delete directly observed surface voxels.
        for idx in remove_indices:
            if idx not in self.observed_indices:
                self.voxel_prob[idx] = self.free_prob

        # Keep directly observed voxels occupied. This prevents noisy over-carving
        # from deleting actual measured surface points.
        ...
        self._log(
            ...
            f"newly free: {newly_free} | occupied volume: {self.fused_volume:.6f} m³ | "
            f"expected volume: {self.expected_volume:.6f} m³"
        )
    ...
    def _initialize_bbox_from_detection(self, points_world):
        ...
        # Initialize the whole bbox as unknown instead of occupied.
        # This avoids creating a solid top slab / solid rectangular block.
        all_bbox_indices = self._fill_bbox()
        self.voxel_prob = {
            idx: self.unknown_prob
            for idx in all_bbox_indices
        }

        # Directly measured surface voxels are occupied.
    ...
    def _compute_bbox(self, points_world):
        ...
        # Margin only in horizontal plane.
        bbox_min[[0, 2]] -= self.bbox_margin
        bbox_max[[0, 2]] += self.bbox_margin

        # The bottom of the initial volume is the floor.
    ...
    def _voxelize(self, points_world):
        ...
        inside = np.all(
            (indices >= 0) & (indices < self.grid_shape),
            axis=1,
        )
        ...
        return {
            tuple(idx)
            for idx in np.unique(indices, axis=0)
        }
    ...
    def _refresh_state_sets_from_probabilities(self):
        ...
        self.unknown_indices = {
            idx
            for idx, prob in self.voxel_prob.items()
            if self.free_prob < prob < self.occupied_threshold
        }
    ...
    def _sync_compatibility_sets(self):
        # For your existing main/plotting code:
        # - extruded_centers and fused_centers now show only occupied voxels.
        # - unknown voxels are stored separately in unknown_centers.
    ...
    def _update_outputs(self):
        ...
        self.extruded_centers = self._indices_to_centers(self.extruded_indices)
        self.fused_centers = self._indices_to_centers(self.fused_indices)
        ...
        self.extruded_volume = len(self.extruded_indices) * voxel_volume
        ...
        # Expected volume treats unknown voxels as 0.5 occupied.
    ...
    def _has_camera_data(depth, fov_degrees, far, position, quaternion):
        return all(
            value is not None
            for value in [depth, fov_degrees, far, position, quaternion]
        )
    ...
    def _log_status(self, title, new_observed=None):
        ...
        message = (
            ...
            f"observed: {len(self.observed_indices)} | "
            f"occupied: {len(self.occupied_indices)} | "
            f"unknown: {len(self.unknown_indices)} | "
            f"free: {len(self.free_indices)} | "
            f"occupied volume: {self.fused_volume:.6f} m³ | "
            f"expected volume: {self.expected_volume:.6f} m³"
        )
    ...
    def get_unknown_surface_indices(self):
        """
        Return unknown voxels that are on the boundary/surface of the unknown region.

        A voxel is an unknown surface voxel if at least one of its 6-neighbors
        is not also unknown.
        """
        ...
        unknown_set = set(self.unknown_indices)
        ...
        for idx in unknown_set:
            ix, iy, iz = idx

            for dx, dy, dz in neighbor_offsets:
                neighbor = (ix + dx, iy + dy, iz + dz)

                if neighbor not in unknown_set:
